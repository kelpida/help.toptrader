...
function iknowledgebase_breadcrumbs() {
	...
	echo ' <div class="col-md-3 text-left"><a class="back-link" href="' . esc_url( home_url() ) . '"><span>' . esc_html__( 'Back', 'iknowledgebase' ) . '</span></a></div>';
	if ( is_category() || is_tag() ) {
		$object = get_queried_object();
		if ( ! empty( $object->parent ) ) {
			$term_id   = $object->parent;
			$term_link = get_term_link( (int) $term_id, $object->taxonomy );
			echo '<div class="col-md-9 text-left is-active"><a class="current-page" href="' . esc_url( $term_link ) . '">' . esc_html( get_the_category_by_ID( (int) $term_id ) ) . '</a><div class="col-md-3">';
		}
